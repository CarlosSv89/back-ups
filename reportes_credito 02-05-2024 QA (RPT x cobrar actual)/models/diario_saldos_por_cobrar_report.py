        ...
        for diario in diarios:
            ...
            for month in facturas_mes:
                ...
                for data in facturas_mes[month]:
                    ...
                    # ------------- month pending amount per client ----------------
                    if data['nombre_cliente'] not in cliente_month_totals:
                        cliente_month_totals[data['nombre_cliente']] = {'client': data['nombre_cliente'],'total_month': data['importe_pendiente']}
                    else:
                        cliente_month_totals[data['nombre_cliente']]['total_month'] += data['importe_pendiente']
                ...
                sheet.merge_range('N3:O3', 'Acumulado por cliente', tuscany_format)
                sheet.write(3, 13, 'Cliente', azure_cell)
                sheet.write(3, 14, 'Total pendiente', azure_cell)
                row = 4
                ...
                # Write total per client
                row_month_total = 4
                
                # Convert dict to list and sort by total_month
                ordered_month_total = [(k,v) for k, v in cliente_month_totals.items()]
                ordered_month_total.sort(key=lambda x: x[1]['total_month'], reverse=True)
                
                for client in ordered_month_total:
                    sheet.write(row_month_total, 13, client[1]['client'])
                    sheet.write(row_month_total, 14, client[1]['total_month'], black_number_cell)
                    row_month_total += 1