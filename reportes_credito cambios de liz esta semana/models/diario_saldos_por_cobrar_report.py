...
class DiarioSaldosPorCobrarReport(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, diarios):
        ...
        for diario in diarios:
            ...
            normal_number_cell = workbook.add_format({'font_color': 'black', 'num_format': '#,##0.00'})
            ...
            for month in facturas_mes:
                ...
                # Sort by importe_pendiente and fecha_vencimiento
                facturas_mes[month] = sorted(facturas_mes[month], key=lambda x: (-x['importe_pendiente'],x['fecha_vencimiento']))
                
                # Group by client
                for data in facturas_mes[month]:
                    if data['nombre_cliente'] not in clients_grouped:
                        clients_grouped[data['nombre_cliente']] = [data]
                    else:
                        clients_grouped[data['nombre_cliente']].append(data)
                
                # Sort dict by sum of importe_pendiente at each client
                clients_grouped = {k: v for k, v in sorted(clients_grouped.items(), key=lambda item: sum(x['importe_pendiente'] for x in item[1]), reverse=True)}
                
                sheet_title = month.replace('_', ' ')
                sheet = workbook.add_worksheet(f'{sheet_title}')
                
                sheet.merge_range('A3:L3', f'Desglose de facturas {sheet_title}', tuscany_format)
                
                # Grouping and hiding columns at first view
                sheet.set_column('A:C', 20, None, {'level': 1, 'hidden': True})
                sheet.set_column(3, 3, 60)
                sheet.set_column('E:F', 20, None, {'level': 1, 'hidden': True})
                sheet.set_column('I:L', 20, None, {'level': 1, 'hidden': True})
                sheet.set_column('G:H', 20, None)
                ...
                # Total and pending amount general per month
                total_general = sum(x['total'] for x in facturas_mes[month])
                pendiente_general = sum(x['importe_pendiente'] for x in facturas_mes[month])
                sheet.write(4, 0, '',)
                sheet.write(4, 1, '')
                sheet.write(4, 2, '')
                sheet.write(4, 3, 'Total general', black_text_cell)
                sheet.write(4, 4, '')
                sheet.write(4, 5, '')
                sheet.write(4, 6, total_general, black_number_cell)
                sheet.write(4, 7, pendiente_general, black_number_cell)
                sheet.write(4, 8, '')
                sheet.write(4, 9, '')
                sheet.write(4, 10, '')
                sheet.write(4, 11, '')
                ...
                # Total per client line
                for client in clients_grouped:
                    total = sum(x['total'] for x in clients_grouped[client])
                    importe_pendiente = sum(x['importe_pendiente'] for x in clients_grouped[client])
                    sheet.write(row, 0, '')
                    sheet.write(row, 1, '')
                    sheet.write(row, 2, '')
                    sheet.write(row, 3, f"TOTAL {client}", black_text_cell)
                    sheet.write(row, 4, '', date_cell)
                    sheet.write(row, 5, '', date_cell)
                    sheet.write(row, 6, total, black_number_cell)
                    sheet.write(row, 7, importe_pendiente, black_number_cell)
                    sheet.write(row, 8, '')
                    sheet.write(row, 9, '')
                    sheet.write(row, 10, '')
                    sheet.write(row, 11, '')
                    row += 1
                    data = clients_grouped[client]
                    
                    # Client invoices lines
                    for fact in data:
                        sheet.set_row(row, None, None, {'level': 1, 'hidden': True})
                        sheet.write(row, 0, fact['concepto'].replace('_', ' '))
                        ...
                        sheet.write(row, 6, fact['total'], normal_number_cell)
                        sheet.write(row, 7, fact['importe_pendiente'], normal_number_cell)
