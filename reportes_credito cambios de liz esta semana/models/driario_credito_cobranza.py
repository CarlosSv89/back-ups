...
class DiarioCreditoCobranza(models.Model):
    ...
    @api.onchange('fecha_inicio', 'fecha_final', 'categoria_rep')
    def _calcular_valores(self):
        for i in self:
            if i.fecha_inicio and i.fecha_final and i.categoria_rep:
                # 
                ...
                while init_date <= i.fecha_final:
                    ...
                    if i.categoria_rep == "credito":
                        clientes = self.env['res.partner'].search([('x_studio_categoria_cliente', '=', 'Crédito'),('servicio_medido', '=', False)])
                        ordenes = self.env['pos.order'].search([('date_order', '>=', init_date.strftime("%Y-%m-%d") + " 07:00:00"), 
                        ('date_order', '<', (init_date + datetime.timedelta(days=1)).strftime("%Y-%m-%d") + " 06:59:59"), ('tipo_pago', '=', 'Crédito'), 
                        ("product_name", "not ilike", "Ajustes por cubicación"), ("product_name", "not ilike", "Compensación"),
                        ("product_name", "not ilike", "Ajuates por cubicación"),("product_name", "not ilike", "Auto consumo"),
                        ("product_name", "not ilike", "Traspasos"),("x_studio_cancelado","!=",True),
                        ('partner_id', 'in', clientes.ids),('partner_id.servicio_medido', '=', False)])
                    if i.categoria_rep == "servicio medido":
                        clientes = self.env['res.partner'].search([('x_studio_categoria_cliente', '=', 'Crédito'),('servicio_medido', '=', True)])
                        ordenes = self.env['pos.order'].search([('date_order', '>=', init_date.strftime("%Y-%m-%d") + " 07:00:00"), 
                        ('date_order', '<', (init_date + datetime.timedelta(days=1)).strftime("%Y-%m-%d") + " 06:59:59"), ('tipo_pago', '=', 'Crédito'), 
                        ("product_name", "not ilike", "Ajustes por cubicación"), ("product_name", "not ilike", "Compensación"),
                        ("product_name", "not ilike", "Ajuates por cubicación"),("product_name", "not ilike", "Auto consumo"),
                        ("product_name", "not ilike", "Traspasos"),("x_studio_cancelado","!=",True),
                        ('partner_id', 'in', clientes.ids),('partner_id.servicio_medido', '=', True)])
