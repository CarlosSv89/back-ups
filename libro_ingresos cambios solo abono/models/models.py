...
class libro_ingresos(models.Model):  # primera clase -----------------------------------------------------
    ...
    def _check_unique_fecha_libro(self):
        if self.fecha_libro:
            for record in self:
                if self.search([('fecha_final', '=', record.fecha_libro), ('planta.name', '=', record.planta.name), ('id', '!=', record.id)], limit=1):
    ...
    def create(self, vals):
        ...
        registro_anterior = self.search([('fecha_final', '=', fecha_anterior), ('planta', '=', planta_id)], limit=1)
        ...
        for libro in self:
            if libro.fecha_libro and libro.fecha_final and libro.planta:
                ...
                registro_anterior = self.env['libro_ingresos.libro'].search(
                    [('fecha_final', '=', fecha_anterior), ('planta', '=', libro.planta.id)], limit=1)
                ...
                cobr = self.env['rpt_linea_cobranza'].sudo().search(
                    [('linea_cobranza.diario_x_dia', '>=', init_date), ('linea_cobranza.diario_x_dia', '<=', end_date), ('linea_cobranza.company_id.name', '=', libro.planta.name)]
                )
                ...
                for cobranza in cobr:
                    linea['fecha'] = cobranza.fecha_cobranza
                    linea['ingreso'] = 'Cobranza'
                    linea['fecha_rda'] = cobranza.fecha_cobranza
                    linea['banco'] = 'BBVA'
                    linea['no_cuenta'] = ''
                    linea['descripcion'] = cobranza.payment_method_id
                    linea['debe'] = cobranza.cobranza
                    linea['haber'] = 0.00
                    saldoaux = saldoaux + cobranza.cobranza
                    linea['saldo'] = saldoaux
                    self.linea_libro_ids = [(0, 0, linea)]
