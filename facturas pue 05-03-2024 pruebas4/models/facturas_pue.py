  ...
  def _get_facturas_pue(self):
    for line in self:
      if line.fecha_inicial and line.fecha_final:
        ...
        # buscar las facturas
        facturas = self.env['account.move'].search([('invoice_date', '>=', line.fecha_inicial),('invoice_date_due', '<=', line.fecha_final),('move_type', '=', 'out_invoice'),('edi_state','in',['to_send','to_cancel']),('state', '=', 'posted'),('x_studio_tipo', '=', 'Crédito')])
...
class Facturas_pue_line(models.Model):
  ...
  company_id = fields.Many2one(string="Empresa",comodel_name='res.company', required=True, default=lambda self: self.env.company)
