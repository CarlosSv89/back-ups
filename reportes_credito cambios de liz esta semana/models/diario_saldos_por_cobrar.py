...
class SaldoPorCobrar(models.Model):
    ...
    all_companies = fields.Boolean(string='Todas las compañias?', default=False)
    ...
    def _calcular_valores(self):
        for i in self:
            if i.fecha_final and i.fecha_inicial:
                ...
                if i.all_companies:
                    companies = self.env['res.company'].search([("x_studio_activo_1", "=", True)])
                    comp_ids = companies.mapped(lambda x: x.id)
                    _logger.error(f"Compañias activas: {len(comp_ids)}")
                    
                    facturas = self.env['account.move'].sudo().search([("company_id", "in", comp_ids),('invoice_date_due', '>=', i.fecha_inicial),('invoice_date_due', '<=', i.fecha_final),('partner_id.x_studio_categoria_cliente', '=', 'Crédito'),('payment_state','in',['not_paid','in_payment','partial']),('move_type', '=', 'out_invoice'),('state', 'not in', ['cancel','draft']),('edi_state','!=','cancelled')])
                    ordenes = self.env['pos.order'].sudo().search([("company_id", "in", comp_ids),('partner_id.x_studio_categoria_cliente', '=', 'Crédito'),('state', '=','done'),('x_studio_cancelado','=',False),('tipo_pago','=','Crédito'),('amount_total','>',0),('x_studio_dias_credito','!=',False)])
                else:
                    facturas = self.env['account.move'].search([('invoice_date_due', '>=', i.fecha_inicial),('invoice_date_due', '<=', i.fecha_final),('partner_id.x_studio_categoria_cliente', '=', 'Crédito'),('payment_state','in',['not_paid','in_payment','partial']),('move_type', '=', 'out_invoice'),('state', 'not in', ['cancel','draft']),('edi_state','!=','cancelled')])
                    ordenes = self.env['pos.order'].search([('partner_id.x_studio_categoria_cliente', '=', 'Crédito'),('state', '=','done'),('x_studio_cancelado','=',False),('tipo_pago','=','Crédito'),('amount_total','>',0),('x_studio_dias_credito','!=',False)])
                ...
                _logger.info(f"Facturas filtradas: {len(facturas)}")
                _logger.info(f"Ordenes filtradas: {len(ordenes)}")
                
                # Cobranza meta
                i.linea_diario_rpt = [(0, 0, self._crear_linea_meta('Cobranza meta', facturas, ordenes, i.fecha_inicial, i.fecha_final))] 
                
                # Descuentos
                i.linea_diario_rpt = [(0, 0, self._crear_linea_descuentos('Descuentos del periodo', facturas, i.fecha_inicial, i.fecha_final))]
    ...
    def _crear_linea_descuentos(self, concepto, facturas, fecha_inicio, fecha_fin):
        ...
        while date_alt <= fecha_fin:
            ...
            if fac_mes:
                for fact in fac_mes:
                    descuentos = json.loads(fact.invoice_payments_widget)
                    if descuentos:
                        for nota in descuentos['content']:
                            if nota['move_type'] == 'out_refund':
                                # _logger.error(f"Widget: {date_alt.month} {nota}")
                                if date_alt.month  == 1:
        ...
        while date_alt <= fecha_fin:
            ...
            for factura in f:
                ...
                if factura.amount_residual_signed != 0.00:
                    ...
                    linea_facturas['company'] = factura.company_id.name
            ...
            for order_vencida in ordenes:
                ...
                if order_vencida_due.date() <= fecha_fin:
                    ...
                    if(orden_valida):
                        _logger.warning(f"Orden valida: {order_vencida}")
                        if date_alt.month == 1:
...
class desgloseFacturasLinea(models.Model):
    ...
    state = fields.Selection(string='Estado', selection=[('draft', 'Borrador'), ('posted', 'Publicado'), ('cancel', 'Cancelado')])
