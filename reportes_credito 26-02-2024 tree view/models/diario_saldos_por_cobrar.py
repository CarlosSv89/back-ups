...
class SaldoPorCobrar(models.Model):
    ...
    @api.onchange('fecha_inicial', 'fecha_final', 'categoria_rep')
    def _calcular_valores(self):
        for i in self:
            if i.fecha_final and i.fecha_inicial and i.categoria_rep:
                ...
                if i.categoria_rep == "credito":
                    facturas = self.env['account.move'].search([('invoice_date_due', '>=', i.fecha_inicial),('invoice_date_due', '<=', i.fecha_final),('move_type', '=', 'out_invoice'),('state', '=', 'posted'),('edi_state','in',['sent','to_cancel']),('x_studio_tipo', '=', 'Crédito'),('partner_id.servicio_medido', '=', False)])
                    ordenes = self.env['pos.order'].search([('state', '=','done'),('x_studio_cancelado','=',False),('tipo_pago','=','Crédito'),('amount_total','!=',0.00),('x_studio_dias_credito','!=',False),('partner_id.servicio_medido', '=', False)])
                if i.categoria_rep == "servicio medido":
                    facturas = self.env['account.move'].search([('invoice_date_due', '>=', i.fecha_inicial),('invoice_date_due', '<=', i.fecha_final),('move_type', '=', 'out_invoice'),('state', '=', 'posted'),('edi_state','in',['sent','to_cancel']),('x_studio_tipo', '=', 'Crédito'),('partner_id.servicio_medido', '=', True)])
                    ordenes = self.env['pos.order'].search([('state', '=','done'),('x_studio_cancelado','=',False),('tipo_pago','=','Crédito'),('amount_total','!=',0.00),('x_studio_dias_credito','!=',False),('partner_id.servicio_medido', '=', True)])
